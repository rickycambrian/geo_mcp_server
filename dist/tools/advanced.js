...
const GraphEntityInputSchema = z.object({
    ...
    values: z
        .array(GraphEntityValueInputSchema)
        .optional()
...
export function registerAdvancedTools(server, session) {
    ...
    // ── build_schema ─────────────────────────────────────────────────────
    server.tool('build_schema', 'Build a complete schema (properties + types) in one call. Creates all properties first, then creates types that reference those properties by name.', {
        properties: z
            .array(z.object({
            name: z.string().describe('Property name'),
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({ properties: createdProperties, types: createdTypes, totalOps }, null, 2),
                },
            ],
        };
    });
    ...
    // ── get_system_ids ───────────────────────────────────────────────────
    server.tool('get_system_ids', 'Get well-known system entity IDs from the Geo knowledge graph. Returns commonly used type, property, and data type IDs.', {
        category: z
 ...
 */
function buildTypedValue(property, type, value) {
    switch (type) {
        case 'text':
            return { property, type: 'text', value: String(value) };
        case 'integer':
            return { property, type: 'integer', value: typeof value === 'number' ? value : Number(value) };
        case 'float':
            return { property, type: 'float', value: typeof value === 'number' ? value : Number(value) };
        case 'boolean':
            return { property, type: 'boolean', value: Boolean(value) };
        case 'date':
            return { property, type: 'date', value: String(value) };
        case 'time':
            return { property, type: 'time', value: String(value) };
        case 'datetime':
            return { property, type: 'datetime', value: String(value) };
        case 'schedule':
            return { property, type: 'schedule', value: String(value) };
        case 'point':
            // Expect value as "lon,lat" string
            const [lon, lat] = String(value).split(',').map(Number);
            return { property, type: 'point', lon, lat };
        default:
            return { property, type: 'text', value: String(value) };
    }
}
