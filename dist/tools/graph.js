...
export function registerGraphTools(server, session) {
    ...
    // ── create_type ──────────────────────────────────────────────────
    server.tool('create_type', 'Create a type (schema) that groups properties', {
    ...
    server.tool('create_entity', 'Create an entity instance in the knowledge graph', {
        ...
        try {
            ...
            if (values !== undefined) {
                params.values = values.map((v) => {
                    if (v.type === 'decimal') {
                        return {
                            ...v,
                            mantissa: { type: 'i64', value: BigInt(v.mantissa) },
                        };
                    }
                    return v;
                });
            ...
            const { id, ops } = Graph.createEntity(params);
    ...
    // ── create_image ─────────────────────────────────────────────────
    server.tool('create_image', 'Create an image entity from a URL', {
    ...
    // ── update_entity ────────────────────────────────────────────────
    server.tool('update_entity', 'Update an existing entity', {
        ...
        try {
            ...
            if (unset !== undefined)
                params.unset = unset;
            const result = Graph.updateEntity(params);
