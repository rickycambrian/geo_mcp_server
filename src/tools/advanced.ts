  ...
  server.tool(
    ...
    async ({
      ...
      try {
        ...
        const arxivId =
          paper.arxivId?.replace(/^arxiv:/i, '').trim()
        ...
        let venueProjectId: string | null = null;
        if (venueName) {
        ...
        const topicKeyToName = new Map<string, string>();
        ...
        if (arxivId) derivedDescriptionParts.push(`arXiv:${arxivId}`);
        if (arxivUrl) derivedDescriptionParts.push(arxivUrl);
        const publicationDate = paper.publicationDate ? coerceDate(paper.publicationDate) : undefined;
        if (publicationDate) derivedDescriptionParts.push(`Published: ${publicationDate}`);
        if (venueName) derivedDescriptionParts.push(`Venue: ${venueName}`);
        ...
        const paperRelations: NonNullable<Parameters<typeof Graph.createEntity>[0]['relations']> = {};
        ...
        for (const claim of claims) {
          ...
          if (topicIdsForClaim.length > 0) {
            ...
            if (names.length > 0) descParts.push(`Topics: ${names.join(', ')}`);
          }
